...
class RealtorSpider(BaseSpider):
    ...
    def start_requests(self):
        yield Request(
            url=self.search_url, callback=self.parse_count, headers=self.headers
        )

    def parse_count(self, response):
        results_xpath = response.xpath(
            "//span[contains(@class, 'jsx-2957100293')]/span/text()"
        ).get()
        if not results_xpath:
            results_xpath = response.xpath(
                "//div[contains(@data-testid, 'total-results')]/text()"
            ).get()
        ...
        for page in range(1, page_count + 1):
            ...
            yield Request(
                url=page_url, callback=self.parse_results, headers=self.headers
            )
